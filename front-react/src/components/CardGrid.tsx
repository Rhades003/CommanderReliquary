    ...
    <div className="grid grid-cols-3 gap-2" style={{display: "grid", gridTemplateColumns:"auto auto auto auto"}}>
      ...
      {cards.map((card) => {
          if (card.card_faces && card.card_faces[0].image_uris) {
            return (
              <>
                <CardSimple key={card.id} id={card.id} image_uris={card.card_faces[0].image_uris} name={card.card_faces[0].name} mana_cost={card.card_faces[0].mana_cost} rarity={card.rarity}  type_line={card.card_faces[0].type_line} action={"rest"} resultForParent={resultForParent}/>
              </>
            );
          } else {
            return (
      ...
      {cards.map((card) => {
        ...
        console.log(card);
          if (card.card_faces && card.card_faces[0].image_uris) {
            return (
              <>
                <CardSimple key={card.id} id={card.id} image_uris={card.card_faces[0].image_uris} name={card.card_faces[0].name} mana_cost={card.card_faces[0].mana_cost} rarity={card.card_faces[0].rarity}  type_line={card.card_faces[0].type_line} action={"sum"} resultForParent={resultForParent}/>
              </>
            );
          } else {
            return (
